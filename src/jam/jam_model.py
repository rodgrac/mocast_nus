...
class MHAttention(nn.Module):
    def __init__(self, heads, head_dim):
        ...
        self.queries = nn.Linear(in_features=self.hidden_size, out_features=self.hidden_size, bias=False)
        self.keys = nn.Conv2d(self.hidden_size, self.hidden_size, kernel_size=1, stride=1, padding=0, bias=False)
        self.values = nn.Conv2d(self.hidden_size, self.hidden_size, kernel_size=1, stride=1, padding=0, bias=False)
        self.attn_out = nn.Linear(in_features=self.hidden_size, out_features=self.hidden_size)

    def forward(self, q, k, v):
        q = self.queries(q).view(q.size(0), self.heads, -1)
        k = self.keys(k).view(k.size(0), self.heads, self.head_dim, -1)
        v = self.values(v).view(v.size(0), self.heads, self.head_dim, -1)
